...
class GestionUsuariosController
{
    ...
    public function postCreate()
    {
        ...
        // --- CAMBIO 1: Reemplazar operador ternario incompleto y ??
        ...
        if (empty($documento) || empty($nombre) || empty($apellido) || empty($telefono) || empty($correo) || empty($password) || empty($id_rol)) {
            // Asumo que jsonResponse es una función global compatible con 5.2.5
        ...
        // Nota: pg_query_params y $1 son específicos de PostgreSQL, asumo que son compatibles con tu entorno (MS4W/PostgreSQL).
        ...
        // --- CAMBIO 2: Reemplazar password_hash (inexistente en 5.2.5)
        $hash = sha1($password); // Usamos sha1 como alternativa común en sistemas legacy

        // --- CAMBIO 3: Reemplazar sintaxis de array corto [] por array()
        $data = array(
            'documento' => $documento,
            'nombre' => $nombre,
            'apellido' => $apellido,
    ...
    {
        $obj = new GestionUsuariosModel();
        // Nota: No se cambia $_GET['id'] porque es compatible.
        $id = $_GET['id'];
        $res = $obj->getById($id);
        // Nota: pg_fetch_assoc es compatible.
    ...
    public function postUpdate()
    {
        ...
        // --- CAMBIO 4: Reemplazar operador Null Coalescing (??)
        $documento = trim(isset($_POST['documento']) ? $_POST['documento'] : '');
        $nombre = trim(isset($_POST['nombre']) ? $_POST['nombre'] : '');
        $apellido = trim(isset($_POST['apellido']) ? $_POST['apellido'] : '');
        $telefono = trim(isset($_POST['telefono']) ? $_POST['telefono'] : '');
        ...
        // --- CAMBIO 5: Reemplazar sintaxis de array corto [] por array()
        $data = array(
            'documento' => $documento,
            'nombre' => $nombre,
            'apellido' => $apellido,
        ...
        // --- CAMBIO 6: Reemplazar password_hash (inexistente en 5.2.5)
    ...
    public function getData()
    {
        ...
        // --- CAMBIO 7: Reemplazar operador Null Coalescing (??)
        ...
        // --- CAMBIO 8: Reemplazar sintaxis de array corto [] por array()
