document.addEventListener('DOMContentLoaded', function () {
    ...
    const modalInhabilitarZoocriadero = document.getElementById('modalInhabilitarZoo');
    if (modalInhabilitarZoocriadero) {
        modalInhabilitarZoocriadero.addEventListener('show.bs.modal', function (event) {
            const button = event.relatedTarget;
            const idZoocriadero = button.getAttribute('data-id');
            const idEstado = button.getAttribute('data-estado'); // Asegúrate de agregar esto al botón

            // Usamos document.getElementById para asegurar que encontramos el input
            const modalInput = document.getElementById('id_zoocriadero');
            const modalText = document.getElementById('zoocriaderoIdText');
            const stateInput = document.getElementById('estado_zoocriadero');

            if (modalInput) {
                modalInput.value = idZoocriadero;
            }
            if (stateInput) {
                stateInput.value = idEstado;
            }
            if (modalText) {
                modalText.textContent = idZoocriadero; // Aquí pones el ID real, no "si"
            }
        });
    }
});