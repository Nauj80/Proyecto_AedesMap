...
// Jsvectormap (inicializar sólo si existe el contenedor) - ejecutar cuando el DOM esté listo y atrapar errores
(function initVectorMap() {
    function run() {
        try {
            var el = document.getElementById('world-map');
            if (!el) return; // nada que hacer si no existe el contenedor
            if (!window.jsVectorMap) { console.warn('jsVectorMap no está cargado, se omite la inicialización del mapa.'); return; }

            var world_map = new window.jsVectorMap({
                selector: "#world-map",
                map: "world",
                zoomOnScroll: false,
                regionStyle: {
            ...
            console.warn('Error inicializando jsVectorMap (omitido):', err);
        }
    }
...
(function(){
    ...
    if (statsEl && typeof statsEl.getContext === 'function') {
        ...
        if (myLegendContainer && typeof statisticsChart.generateLegend === 'function') {
    ...
    if (dailyEl && typeof dailyEl.getContext === 'function') {
        var dailySalesChart = dailyEl.getContext('2d');
