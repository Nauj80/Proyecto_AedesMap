...
*/
  ...
  if (browser.isIE) {
    _tagMain.onselectstart = function () {
      return false;
    };
    _tagMain.ondrag = function () {
      return false;
    };
  } else {
  ...
  this.debug = function () {
    ...
    add_event(db, "click", function () {
      prompt("Debug INFO", i.get_map_url());
    });
  };
  ...
  this.isPointInMap = function (p_x, p_y, p_w, p_h) {
    if (
      p_x > _ext_Xmax + i.wPixel2real(_map_w_bord) ||
      p_x + p_w < _ext_Xmin - i.wPixel2real(_map_w_bord)
    )
      return false;
    if (
      p_y - p_h > _ext_Ymax + i.hPixel2real(_map_h_bord) ||
      p_y + p_h - p_h < _ext_Ymin - i.hPixel2real(_map_h_bord)
    )
      return false;
    return true;
  };
  ...
  this.setReferenceBox = function (p_Xmin, p_Xmax, p_Ymin, p_Ymax) {
    ...
    Xmin = i.xReal2pixel(p_Xmin);
    Ymin = i.yReal2pixel(p_Ymin);
    Xmax = i.xReal2pixel(p_Xmax);
    Ymax = i.yReal2pixel(p_Ymax);
  ...
  this.setFullExtent = function (Xmin, Xmax, Ymin) {
    ...
    _ext_Ymin_orig = Ymin;
    _ext_Ymax_orig =
      (_map_h / _map_w) * (_ext_Xmax_orig - _ext_Xmin_orig) + _ext_Ymin_orig;
  ...
  this.setZoomboxWH = function (x, y) {
    ...
    _tagZoombox.style.width =
      max(_zoombox_x_last, _zoombox_x_first) -
      min(_zoombox_x_last, _zoombox_x_first) +
      "px";
    _tagZoombox.style.height =
      max(_zoombox_y_last, _zoombox_y_first) -
      min(_zoombox_y_last, _zoombox_y_first) +
      "px";
  ...
  this.redraw = function (redrawAttached) {
    ...
    if (_referenceMap != null) {
      // Draw zoom box in the reference map
  ...
  this.mapLoaded = function () {
    ...
    _tagOverlay.style.left = "0";
    _tagOverlay.style.top = "0";
  ...
  this.get_map_url = function () {
    ...
    if (_protocol == "mapservercgi") {
      var size =
        "mapsize=" +
        (_map_w + _map_w_bord + _map_w_bord) +
        "+" +
        (_map_h + _map_h_bord + _map_h_bord);
      var ext =
        "mapext=" +
        (_ext_Xmin - i.wPixel2real(_map_w_bord)) +
        "+" +
  ...
  this.init = function () {
    ...
    _tagMain.oncontextmenu = function () {
      return false;
    };
    ...
    _tagEvents.oncontextmenu = function () {
      return false;
    };
    ...
    _tagMap.objRef = i;
    _tagMap.oncontextmenu = function () {
      return false;
    };
    _tagMap.onmousedown = function () {
      return false;
    }; // Disable drag'n drop
    ...
    _tagMap.style.margin = "0";
    _tagMap.style.padding = "0";
    ...
    _tagZoombox.oncontextmenu = function () {
      return false;
    };
    ...
    // "Loading" image tag
    _tagLoading.oncontextmenu = function () {
      return false;
    };
    _tagLoading.onmousedown = function () {
      return false;
    }; // Disable drag'n drop
    ...
    _tagLoading.style.margin = "0";
    _tagLoading.style.padding = "0";
  ...
  this.dragGo = function (event) {
    ...
    dragObj.elNode.style.left =
      dragObj.elStartLeft + xx - dragObj.cursorStartX + "px";
    dragObj.elNode.style.top =
      dragObj.elStartTop + yy - dragObj.cursorStartY + "px";
    // Move Overlay layer
    _tagOverlay.style.left =
      parseInt(dragObj.elNode.style.left) + _map_w_bord + "px";
    _tagOverlay.style.top =
      parseInt(dragObj.elNode.style.top) + _map_h_bord + "px";
  ...
  this.removeOverlayPoints = function () {
    ...
    for (var j = kids.length - 1; j >= 0; j--) {
      _tagPoints.removeChild(kids[j]);
    }
  };

  this.overlayPointsResort = function () {
    // Z sorting
    _pointsOverlayArray.sort(function (a, b) {
      if (a.getY() > b.getY()) {
        return -1;
      } else {
        return 1;
...
function DL_GetElementLeft(eElement) {
  ...
  while (eParElement != null) {
    ...
    } // if browser is Gecko, then...
    else {
      if (eParElement.tagName == "TABLE") {
        // if parent is a table, then...
...
function delCookie(p_name) {
  ...
  document.cookie =
    p_name + "=" + "; expires=Thu, 01-Jan-70 00:00:01 GMT" + "; path=/";
}
...
function pointOverlay(
  ...
  if (_infoSkin == null) {
    // Create a default Info-window icon object...
    _infoSkin = new msInfoSkin(
      "misc/img/angolo_a.png",
      "misc/img/angolo_b.png",
      "misc/img/angolo_c.png",
      "misc/img/angolo_d.png",
      "misc/img/report_t.png",
      "misc/img/report_d.png",
      "misc/img/report_l.png",
      "misc/img/report_r.png",
      "misc/img/report_x.png",
      "misc/img/close.png",
  ...
  this.getHtmlAttributes = function () {
    ...
    for (var j = 0; j < _item_name.length; j++) {
      // css
      ret +=
        '<tr><td class="mscross_report_attr_name">' +
        _item_name[j] +
  ...
  this.getInfoX = function () {
    return _msMap.xReal2pixel(_x);
  };
  this.getInfoY = function () {
    return Math.round(_msMap.yReal2pixel(_y) - parseInt(_img.offsetHeight) / 2);
  };
  this.getInfoSkin = function () {
    return _infoSkin;
  };
  this.getWidth = function () {
    return parseInt(_img.style.width);
  };
  this.getHeight = function () {
    return parseInt(_img.style.height);
  };
  ...
  this.redraw = function () {
    ...
    if (
      _msMap.isPointInMap(
        _x - _msMap.wPixel2real(_offsetX),
      ...
      setPos(
        _img,
        _msMap.xReal2pixel(_x) - _offsetX,
  ...
  add_event(_img, "click", function (event) {
    pointOverlayEvent(event);
  });
}
...
function msInfoSkin(
  ...
  var _corner_a = new Image();
  _corner_a.src = p_corner_a;
  var _corner_b = new Image();
  _corner_b.src = p_corner_b;
  var _corner_c = new Image();
  _corner_c.src = p_corner_c;
  var _corner_d = new Image();
  _corner_d.src = p_corner_d;

  var _bord_top = new Image();
  _bord_top.src = p_top;
  var _bord_bottom = new Image();
  _bord_bottom.src = p_bottom;
  var _bord_left = new Image();
  _bord_left.src = p_left;
  var _bord_right = new Image();
  _bord_right.src = p_right;
  ...
  this.getCornerA = function () {
    return _corner_a.src;
  };
  this.getCornerB = function () {
    return _corner_b.src;
  };
  this.getCornerC = function () {
    return _corner_c.src;
  };
  this.getCornerD = function () {
    return _corner_d.src;
  };
  this.getFill = function () {
    return _fill.src;
  };
  this.getLeft = function () {
    return _bord_left.src;
  };
  this.getRight = function () {
    return _bord_right.src;
  };
  this.getTop = function () {
    return _bord_top.src;
  };
  this.getBottom = function () {
    return _bord_bottom.src;
  };
...
function msIcon(p_img, p_shd, p_offsetX, p_offsetY) {
  ...
  if (_img_name == "") {
    _img_name = "misc/img/pixel.gif";
  }
  if (_shd_name == "") {
    _shd_name = "misc/img/pixel.gif";
  }

  this.getShiftX = function () {
    return _offsetX;
  };
  this.getShiftY = function () {
    return _offsetY;
  };
  this.getImage = function () {
    var tmp = document.createElement("img");
    tmp.oncontextmenu = function () {
      return false;
    };
...
function msReport(p_pnt, p_title) {
  ...
  this.setContent = function (p_html) {
    _content.innerHTML = p_html;
  };
  ...
  this.init = function () {
    ...
    add_event(close, "click", function () {
      j.close();
    });
...
function parsePointsFromGML(myxml) {
  ...
  for (var i = 0; i < count; i++) {
    _coords =
      myxml.getElementsByTagName(featureMember_Name)[i].childNodes[0 + _add]
        .childNodes[0 + _add].childNodes[0 + _add].childNodes[0 + _add]
    ...
    var size =
      (myxml.getElementsByTagName(featureMember_Name)[0].childNodes[0 + _add]
        .childNodes.length -
        _add) /
      _molt;
    ...
    for (var j = 2; j < size; j++) {
      nam =
        myxml.getElementsByTagName(featureMember_Name)[i].childNodes[0 + _add]
          .childNodes[j * _molt + _add].tagName;
      var nam = nam.split(":");

      val =
        myxml.getElementsByTagName(featureMember_Name)[i].childNodes[0 + _add]
          .childNodes[j * _molt + _add].childNodes[0].nodeValue;
...
function msTool(p_title, p_event_button, p_icon, p_event_map) {
  ...
  if (p_event_button != null) {
    _event_button = function (e) {
      p_event_button(e, _map);
    };
  }
  ...
  if (p_event_map != null) {
    _event_map = function (e) {
      var xx = _map.getClick_X(e);
      var yy = _map.getClick_Y(e);
  ...
  _tag.oncontextmenu = function () {
    return false;
  };
  _tag.onmousedown = function () {
    return false;
  }; // Disable drag'n drop
  ...
  this.setMap = function (p) {
    _map = p;
  };
  this.setToolbar = function (p) {
    _toolbar = p;
  };
  this.haveMapEvent = function () {
    if (_event_map == null) {
      return false;
    }
    return true;
...
function msToolbar(p_msMap, _control, _default) {
  ...
  this.redraw = function () {
    if (
      ...
      box.style.left =
        parseInt(_tagMap.style.width) - (40 + _msMap.getBorder() * 2) + "px";
      box.style.top = "0px";
      box.style.width = "40px";
      box.style.height = _tagMap.style.height;
      for (i = 0; i < _toolbarArray.length; i++) {
        setPos(
          _toolbarArray[i].getTag(),
          parseInt(box.style.left) + 5,
          i * 40 + 5
        );
      }
      //{ setPos(_toolbarArray[i].getTag(), 1400,1400); }
    }
  ...
  _tagToolbar.oncontextmenu = function () {
    return false;
  };
  ...
  if (_default == true) {
    var t_fullext = new msTool(
      "Full Extent",
      _msMap.fullExtent,
      _iconFullExtentButton
    );
    var t_pan = new msTool(
      "Pan",
      _msMap.setActionPan,
      _iconPanButton,
      function (e, map, x, y) {
    ...
    var t_zoom = new msTool(
      "Zoom",
      _msMap.setActionZoombox,
      _iconZoomboxButton,
      function (e, map, x, y) {
        map.zoomStart(e);
      }
    );
    var t_zoomin = new msTool(
      "Zoom In",
      _msMap.setActionZoomIn,
      _iconZoominButton
    );
    var t_zoomout = new msTool(
      "Zoom Out",
      _msMap.setActionZoomOut,
      _iconZoomoutButton
    );
